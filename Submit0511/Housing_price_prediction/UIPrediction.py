...
class UIPrediction:
    ...
    def create_ui(self):
        ...
        main_panel.pack(fill=X, side=TOP, expand=False)
        ...
        designedby_label = Label(designedby_panel, text="Designed by: Vay Thuong Thu")
    ...
    def do_evaluation(self):
        # print the intercept
        print(self.ln.intercept_)
        insert_text=self.ln.intercept_

        self.coeff_df = pd.DataFrame(self.ln.coef_, self.X.columns, columns=['Coefficient'])
        print(self.coeff_df)
        self.coefficient_detail_text.insert(END, self.coeff_df)

        predictions = self.ln.predict(self.X_test)
        print(predictions)
        print("self.X_test")
        print(self.X_test)
        print("self.y_test")
    ...
    def do_save_model(self):
        ...
        confirm = messagebox.askyesno(
            ...
            "Bạn có chắc chắn muốn lưu mô hình này?\n\n"
            f"Training Rate: {self.training_rate.get()}%\n"
            f"MAE: {self.mae_value.get():.2f}\n"
            f"MSE: {self.mse_value.get():.2f}\n"
            f"RMSE: {self.rmse_value.get():.2f}")

        if not confirm:
            messagebox.showinfo("Thông báo", "Lưu mô hình đã bị hủy")
            return
        ...
        # Cập nhật OptionMenu với mô hình mới
        self.model_menu.children["menu"].add_command(
            label=filename,
            command=lambda md=filename: self.selected_model.set(md))

        messagebox.showinfo("Thành công", f"Mô hình đã được lưu thành công!\n\nTên file: {filename}")
        pass
    ...
    def do_load_model(self):
        ...
        # Kiểm tra xem người dùng đã chọn mô hình hay chưa
        if trainedModelName.startswith("----"):
            messagebox.showwarning(
                "Cảnh báo",
                "Bạn phải chọn một mô hình đã được lưu trước khi nạp"
            )
            return

        # Kiểm tra xem file có tồn tại không
        model_path = os.path.join("model", trainedModelName)
        if not os.path.exists(model_path):
            messagebox.showerror(
                "Lỗi",
                f"Không tìm thấy file: {trainedModelName}"
            )
            return
        ...
        try:
            ...
            messagebox.showinfo(
                "Thành công",
                f"Mô hình đã được nạp thành công!\n\nTên file: {trainedModelName}"
            )
        except Exception as e:
            messagebox.showerror(
                "Lỗi",
                f"Lỗi khi nạp mô hình: {str(e)}"
            )
        pass

    def do_prediction(self):
        try:
            # Kiểm tra xem mô hình đã được nạp hay chưa
            if not hasattr(self, 'lm') or self.lm is None:
                messagebox.showerror(
                    "Lỗi",
                    "Vui lòng nạp một mô hình trước khi thực hiện dự đoán"
                )
                return

            # Lấy giá trị từ các input field
            try:
                area_income = float(self.area_income_value.get())
                area_house_age = float(self.area_house_age_value.get())
                area_number_of_rooms = float(self.area_number_of_rooms_value.get())
                area_number_of_bedrooms = float(self.area_number_of_bedrooms_value.get())
                area_population = float(self.area_population_value.get())
            except ValueError:
                messagebox.showerror(
                    "Lỗi nhập liệu",
                    "Vui lòng nhập các giá trị số hợp lệ cho tất cả các trường!\n\n"
                    "Kiểm tra:\n"
                    "- Avg. Area Income\n"
                    "- Avg. Area House Age\n"
            ...
            # Thực hiện dự đoán
            result = self.lm.predict([[area_income, area_house_age,
                                       area_number_of_rooms, area_number_of_bedrooms,
                                       area_population]])

            # Đặt giá trị dự đoán
            prediction_price = result[0]
            self.prediction_price_value.set(prediction_price)

            # Hiển thị thông báo thành công với chi tiết
            messagebox.showinfo(
                "Dự đoán thành công",
                f"Giá nhà dự đoán: ${prediction_price:,.2f}"
            )

        except AttributeError as e:
            messagebox.showerror(
                "Lỗi",
                f"Vui lòng nạp một mô hình trước khi thực hiện dự đoán\n\nChi tiết: {str(e)}"
            )
        except Exception as e:
            messagebox.showerror(
                "Lỗi không mong muốn",
